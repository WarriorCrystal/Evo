...
public class AutoSalDupe extends Module {
    public AutoSalDupe() {
        super("AutoSalDupe", Category.MISC);
    }
    ...
    public void setup(){
        Evo.getInstance().settingsManager.rSetting(delay = new Setting("Delay", this, 1.5f, 0.1f, 15.0f, true));
        Evo.getInstance().settingsManager.rSetting(hitGround = new Setting("HitGround", this, true));
        Evo.getInstance().settingsManager.rSetting(shulkerOnly = new Setting("ShulkerOnly", this, true));
    }
    ...
    public void onUpdate() {
        ...
        if (!timer.passed((float) delay.getValDouble() * 100f))
            return;
        ...
        if (mc.currentScreen instanceof GuiScreenHorseInventory) {
            ...
            for (int l_I = 2; l_I < l_Chest.horseInventory.getSizeInventory() + 1; ++l_I) {
                ...
                if (shulkerOnly.getValBoolean() && !(l_Stack.getItem() instanceof ItemShulkerBox))
                    continue;
    ...
    private void HandleStoring(int p_WindowId, int p_Slot)
    {
        for (int l_Y = 9; l_Y < mc.player.inventoryContainer.inventorySlots.size() - 1; ++l_Y)
        {
            ...
            if (!(l_InvStack.getItem() instanceof ItemShulkerBox) && shulkerOnly.getValBoolean())
                continue;
