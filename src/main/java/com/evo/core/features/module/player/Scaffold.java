...
public class Scaffold extends Module {
    ...
    public void onUpdate() {
        ...
        if (BlockUtil.isClickable(below)) {
            ...
            if (placeTimer.passedMs(placeDelay.getValue().longValue())) {
                ...
                getEvo().getInteractionManager().place(below, place.getValue(), rotate.getValue(), swing.getValue());
    ...
    private void swapBack() {
        if (oldSlot != -1) {
            getEvo().getInventoryManager().swap(oldSlot, swap.getValue().swap);
            oldSlot = -1;
        }
    ...
    private boolean handleSwapping() {
        ...
        if (!InventoryUtil.isHolding(ItemBlock.class, offhand.getValue())) {
            if (!swap.getValue().equals(ScaffoldSwap.NONE)) {
                ...
                getEvo().getInventoryManager().swap(slot, swap.getValue().swap);
