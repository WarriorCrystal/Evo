...
import cf.warriorcrystal.evo.util.salhack.*;
import cf.warriorcrystal.evo.util.salhack.BlockInteractionHelper.PlaceResult;
import cf.warriorcrystal.evo.util.salhack.MinecraftEvent.Era;
...
public class AutoBuild extends Module {
    public AutoBuild() {
        super("AutoBuild", Category.MISC);
    }

    Setting Mode;
    Setting BuildingMode;
    ...
    public void setup(){
        ...
        Modes.add("Cover");
        Modes.add("Wall");
        Modes.add("HighwayWall");
        Modes.add("Stair");
        ArrayList<String> BuildingModes = new ArrayList<>();
        BuildingModes.add("Dynamic");
        BuildingModes.add("Static");
        Evo.getInstance().settingsManager.rSetting(Mode = new Setting("Mode", this, "Flat", Modes));
        Evo.getInstance().settingsManager.rSetting(BuildingMode = new Setting("BuildingMode", this, "Dynamic", BuildingModes));
        Evo.getInstance().settingsManager.rSetting(BlocksPerTick = new Setting("BlocksPerTick", this, 4, 1, 10, true));
        Evo.getInstance().settingsManager.rSetting(Delay = new Setting("Delay", this, 0f, 0.0f, 1.0f, false));
        Evo.getInstance().settingsManager.rSetting(Visualize = new Setting("Visualize", this, true));
    }

    private Vec3d Center = Vec3d.ZERO;
    private ICamera camera = new Frustum();
    private cf.warriorcrystal.evo.util.salhack.Timer timer = new cf.warriorcrystal.evo.util.salhack.Timer();
    ...
    private Listener<EventPlayerMotionUpdate> OnPlayerUpdate = new Listener<>(p_Event ->
    {
        ...
        if (!timer.passed((float) Delay.getValDouble() * 1000f))
            return;
        ...
        if (BuildingMode.getValString() == "Dynamic")
            BlockArray.clear();
        ...
        if (slot != -1)
        {
            if ((mc.player.onGround))
            {
                ...
                int l_BlocksPerTick = BlocksPerTick.getValInt();
        ...
        if (!l_NeedPlace && Mode.getValString() == "Portal")
        {
        ...
        else if (l_NeedPlace && Mode.getValString() == "Portal")
            NetherPortalTimer.reset();
