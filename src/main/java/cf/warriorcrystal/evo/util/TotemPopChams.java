...
import cf.warriorcrystal.evo.module.modules.render.PopChams;
...
public class TotemPopChams {
    ...
    public void onRenderWorld(final RenderWorldLastEvent event) {
        ...
        Color lineColorS = new Color(PopChams.rL.getValInt(), PopChams.bL.getValInt(), PopChams.gL.getValInt(), PopChams.aL.getValInt());
        Color fillColorS = new Color(PopChams.rF.getValInt(), PopChams.bF.getValInt(), PopChams.gF.getValInt(),PopChams. aF.getValInt());
        int lineA = lineColorS.getAlpha();
        int fillA = (fillColorS).getAlpha();
        final long time = System.currentTimeMillis() - this.startTime - ((Number)PopChams.fadestart.getValInt()).longValue();
        if (System.currentTimeMillis() - this.startTime > ((Number)PopChams.fadestart.getValInt()).longValue()) {
            double normal = this.normalize((double)time, 0.0, ((Number)PopChams.fadetime.getValDouble()).doubleValue());
        ...
        if (this.player != null && this.playerModel != null) {
            ...
            GL11.glEnable(2881);
            GL11.glEnable(2848);
            if (alphaFill > 1) alphaFill -= PopChams.fadetime.getValDouble();
            Color fillFinal = new Color(fillColor.getRed(), fillColor.getGreen(), fillColor.getBlue(), (int)alphaFill);

            if (alphaLine > 1) alphaLine -= PopChams.fadetime.getValDouble();
